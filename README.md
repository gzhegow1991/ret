...
## Pattern `Result<T,E> # Result|Either # Ret Object`
...
Представляю вам концепцию Result<T,E> на языке PHP. Для чего он нужен? Вы пишете одну функцию проверяющую тип и указываете текст ошибки. Потом из своего кода вы не пишете if/throw, а просто вызываете метод с пометкой orThrow() или orFallback(). Ваш метод отныне может и исключения бросать и одновременно быть проверялкой какого-нибудь типа. С помощью false/null цепочек можно управлять - собрать все ошибки или собрать все результаты. Удобно? Да.

К нему идут ошибки, допиленные исключения, чтобы логировать и выводить их, ErrorBag, чтобы удобно их накапливать и работать с тегами, ну и Wrapper для легаси-кода.

Кроме того, вы можете сделать `Err::debug(true)`, и все ваши ошибки при создании будут под капотом создавать ещё и исключение (не бросая его) - и выполучите полный трейс до места, где ваша проблема возникла
