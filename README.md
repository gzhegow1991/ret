...
Представляю вам концепцию Result<T,E> на языке PHP. К нему идут ошибки, допиленные исключения, чтобы логировать и выводить их, ErrorBag, чтобы удобно их накапливать и работать с тегами, ну и Wrapper для легаси-кода.

Для чего он нужен? Вы пишете одну функцию проверяющую тип и указываете текст ошибки. Потом из своего кода вы не пишете if/throw, а просто вызываете метод с пометкой orThrow() или orFallback(). Ваш метод отныне может и исключения бросать и одновременно быть проверялкой какого-нибудь типа. С помощью false/null цепочек можно управлять - собрать все ошибки или собрать все результаты. Удобно? Да.
