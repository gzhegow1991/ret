#  `Result<T,E>` / `Result|Either` / `Ret Object`
...
Представляю вам концепцию Result<T,E> на языке PHP. К нему идут допиленные исключения, ошибки, позволяющие их логировать и выводить наравне с ними, а также ErrorBag, чтобы удобно их накапливать и работать с потомками.

Для чего он нужен? Вы пишете одну функцию проверяющую тип и указываете текст ошибки. Потом из своего кода вы не пишете if/throw, а просто вызываете метод с пометкой orThrow() или orFallback(). Ваш метод отныне может и исключения бросать и одновременно быть проверялкой какого-нибудь типа. В довесок - реализовал полную булеву логику для этих объектов NOT/OR/AND/XOR/NXOR

Посмотрите тесты, будет что вспомнить.
...
[ "{ object # Gzhegow\Ret\ErrorBag\TaggedError }", "{ object # Gzhegow\Ret\Error\PHP8\MainError }", [ "tag1" => TRUE, "tag3" => TRUE ] ]
[ "{ object # Gzhegow\Ret\ErrorBag\TaggedError }", "{ object # Gzhegow\Ret\Error\PHP8\MainError }", [ "tag2" => TRUE, "tag3" => TRUE ] ]
[ "{ object # Gzhegow\Ret\ErrorBag\TaggedError }", "{ object # Gzhegow\Ret\Error\PHP8\AggregateError }", [ "tag3" => TRUE ] ]
...
[ "{ object # Gzhegow\Ret\ErrorBag\TaggedError }", "{ object # Gzhegow\Ret\Error\PHP7\MainError }", [ "tag1" => TRUE, "tag3" => TRUE ] ]
[ "{ object # Gzhegow\Ret\ErrorBag\TaggedError }", "{ object # Gzhegow\Ret\Error\PHP7\MainError }", [ "tag2" => TRUE, "tag3" => TRUE ] ]
[ "{ object # Gzhegow\Ret\ErrorBag\TaggedError }", "{ object # Gzhegow\Ret\Error\PHP7\AggregateError }", [ "tag3" => TRUE ] ]
